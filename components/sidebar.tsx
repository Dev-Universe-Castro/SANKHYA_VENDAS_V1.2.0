...
import { Home, Users, ChevronLeft, ChevronRight, LogOut, UserCircle, LayoutGrid, Package, ShoppingCart, Calendar, DollarSign, Settings, Download, Loader2, Cloud, CloudOff, Menu, Route } from "lucide-react"
...
export default function Sidebar({ isOpen, onClose, isCollapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "hidden lg:flex fixed inset-y-0 left-0 z-30 bg-sidebar transform transition-all duration-200 ease-in-out flex-col h-screen",
        isCollapsed ? "w-20 min-w-[80px] max-w-[80px]" : "w-64 min-w-[256px] max-w-[256px]",
      )}
    >
      {/* Logo */}
      <div className="border-b border-sidebar-border shrink-0 relative bg-white">
        <div
          className={cn(
        ...
        >
          <div className={cn("flex items-center", isCollapsed ? "justify-center w-full" : "gap-3")}>
            {isCollapsed ? (
              <Image src="/logo-icon-collapsed.png" alt="PredictSales" width={48} height={48} className="object-contain scale-110" />
            ) : (
              <div className="flex items-center justify-center w-full py-2">
                <Image
                  src="/logo-predictsales-expanded.png"
                  alt="PredictSales"
                  width={200}
                  height={80}
        ...
        <Button
          ...
          className="hidden lg:flex text-sidebar-foreground hover:bg-sidebar-accent absolute -right-3 -bottom-3 bg-sidebar border border-sidebar-border rounded-full w-6 h-6 p-0 z-10 shadow-sm"
        >
      ...
      <div className="hidden lg:block p-3 border-t border-sidebar-border shrink-0">
        <Button
          ...
          className={cn(
            "w-full flex items-center gap-2.5 px-2.5 py-2 text-sm font-medium text-sidebar-foreground/80 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground whitespace-nowrap h-9",
