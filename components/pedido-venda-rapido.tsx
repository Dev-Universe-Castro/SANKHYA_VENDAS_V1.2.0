...
export default function PedidoVendaRapido({ isOpen, onClose, pedidoBase }: PedidoVendaRapidoProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-full w-full h-full md:max-w-[95vw] md:h-[95vh] p-0 overflow-hidden flex flex-col m-0 rounded-none md:rounded-lg" showCloseButton={true}>
        <style jsx>{`
          .scrollbar-hide {
        ...
        {/* Footer Fixo com Totais */}
        {pedido && (
          <div className="border-t px-3 md:px-6 py-3 md:py-4 flex-shrink-0 bg-white shadow-lg">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              {/* Resumo */}
              <div className="flex items-center gap-2 md:gap-3">
                <div className="p-2 md:p-2 bg-green-50 rounded-lg flex-1 md:flex-none min-w-0">
                  <p className="text-[10px] md:text-[10px] text-green-600 font-medium leading-tight">Itens</p>
                  <p className="text-sm md:text-base font-bold text-green-700 leading-tight">
                    {pedido.itens?.length || 0}
                  </p>
                </div>
                <div className="p-2 md:p-2 bg-green-50 rounded-lg flex-1 md:flex-none min-w-0">
                  <p className="text-[10px] md:text-[10px] text-green-600 font-medium leading-tight">Qtd</p>
                  <p className="text-sm md:text-base font-bold text-green-700 leading-tight">
                    {(pedido.itens || []).reduce((sum: number, item: any) => sum + (Number(item.QTDNEG) || 0), 0)}
                  </p>
                </div>
                <div className="p-2 md:p-2 bg-green-600 rounded-lg flex-1 md:flex-none min-w-0">
                  <p className="text-[10px] md:text-[10px] text-white font-medium leading-tight">Total</p>
                  <p className="text-sm md:text-base font-bold text-white leading-tight">{formatCurrency(calcularTotalPedido())}</p>
                </div>
              </div>
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={!pedido || !pedido.itens || pedido.itens.length === 0}
              >
                {loading ? (
                  <>
        ...
        <div className="md:hidden fixed bottom-20 right-4 z-40">
          <Button
            onClick={(e) => {
              e.preventDefault()
              e.stopPropagation()
              console.log('🛒 [CARRINHO FLUTUANTE RÁPIDO] Abrindo carrinho - Total itens:', pedido?.itens?.length)
