...
interface CarrinhoPedidoRapidoProps {
  ...
  editarItem: (index: number, novoItem: any) => void // Modificado para aceitar o novo item
  onCancelar: () => void
  onCriarPedido: () => void
  isOpen: boolean
  onClose: () => void
...
export function CarrinhoPedidoRapido({
  ...
  const handleEditarClick = async (item: any, index: number) => {
    ...
    try {
      ...
      const unidades: UnidadeVolume[] = [
        {
          CODVOL: item.UNIDADE || 'UN',
          DESCRICAO: `${item.UNIDADE || 'UN'} - Unidade Padrão`,
          QUANTIDADE: 1,
          isPadrao: true
        },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-full w-full h-full md:max-w-md md:max-h-[90vh] p-0 m-0 rounded-none md:rounded-lg">
        {/* Header */}
        <DialogHeader className="border-b p-4 flex-shrink-0 bg-green-600 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-white">Carrinho - Pedido Rápido</DialogTitle>
      ...
      <ConfiguracaoProdutoModal
        ...
        configInicial={{
          ...
          desconto: produtoEditando?.PERCDESC,
