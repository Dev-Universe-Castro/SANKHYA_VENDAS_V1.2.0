...
export default function PedidoVendaFromLead({
  ...
  const [itens, setItens] = useState<ItemPedido[]>(() => {
    ...
    if (dadosIniciais.itens && Array.isArray(dadosIniciais.itens) && dadosIniciais.itens.length > 0) {
      return dadosIniciais.itens.map((item: any, index: number) => ({
        ...
        // Garantir que VLRDESC e VLRTOT estejam presentes, mesmo que zerados inicialmente
  ...
  useEffect(() => {
    if (!Array.isArray(itens)) {
      console.warn('⚠️ itens não é um array:', itens)
      setItens([]) // Garantir que seja um array vazio
      return
    }
    ...
    // Forçar atualização do pedido para que o rodapé seja re-renderizado
    setPedido(prev => ({
      ...prev,
      VLRNOTA: total,
      itens: itens // Garantir que pedido.itens está sincronizado
    }))
  }, [itens, itens.length])
  ...
  const handleConfirmarProdutoEstoque = async (produto: any, preco: number, quantidade: number, desconto?: number) => {
    // Fechar modais
  ...
  const calcularTotal = (item: ItemPedido) => {
    ...
    const desconto = total * (percdesc / 100)
    return total - desconto
  }

  const calcularTotalPedido = () => {
    const itensArray = Array.isArray(itens) ? itens : []
    const total = itensArray.reduce((acc, item) => acc + calcularTotal(item), 0)
    console.log('💰 Total do pedido calculado:', total, '| Itens:', itensArray.length)
    return total
  }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* Header com Título e Carrinho - Mobile e Desktop */}
      <div className="flex-shrink-0 bg-white md:bg-gradient-to-r md:from-green-600 md:to-green-500 border-b px-4 py-4 md:px-6 md:py-4 shadow-sm">
        <div className="flex items-center justify-between gap-3 md:gap-6">
          <div className="flex-1 min-w-0">
            <h2 className="text-lg md:text-xl font-bold text-gray-900 md:text-white">
              {isLeadVinculado ? 'Criar Pedido - Lead Ganho' : 'Pedido de Venda'}
            </h2>
            <p className="text-sm md:text-base text-gray-600 md:text-white/90 mt-1">
              {pedido.RAZAO_SOCIAL || 'Selecione um parceiro'}
            </p>
          </div>

          {/* Botão Carrinho - Mobile e Desktop */}
          <Button
            variant="outline"
            size="icon"
            onClick={(e) => {
            ...
            className="border-2 border-gray-300 md:border-white text-gray-700 md:text-white hover:bg-gray-100 md:hover:bg-green-700 relative flex-shrink-0 h-12 w-12 md:h-12 md:w-12 md:mr-8"
          >
            <ShoppingCart className="w-6 h-6" />
            {itens && Array.isArray(itens) && itens.length > 0 && (
              <Badge className="absolute -top-2 -right-2 h-6 w-6 flex items-center justify-center p-0 bg-red-500 text-white text-xs font-bold border-2 border-white rounded-full">
                {itens.length}
              </Badge>
            )}
          </Button>
      ...
      {/* Abas de Navegação */}
      <div className="flex-shrink-0 bg-white border-b shadow-sm mt-0">
        {/* Tabs de Navegação */}
        <Tabs defaultValue="cabecalho" className="w-full">
          <TabsList className="grid w-full grid-cols-6 h-auto gap-0.5 rounded-none bg-gray-100">
            <TabsTrigger value="cabecalho" className="data-[state=active]:bg-green-600 data-[state=active]:text-white text-xs md:text-sm px-1 md:px-2 py-2 md:py-2.5 rounded-none">
              Cabeçalho
            </TabsTrigger>
            <TabsTrigger value="catalogo" className="data-[state=active]:bg-green-600 data-[state=active]:text-white text-xs md:text-sm px-1 md:px-2 py-2 md:py-2.5 rounded-none">
              Catálogo
            </TabsTrigger>
            <TabsTrigger value="mixprodutos" className="data-[state=active]:bg-green-600 data-[state=active]:text-white text-xs md:text-sm px-1 md:px-2 py-2 md:py-2.5 rounded-none">
              Mix IA
            </TabsTrigger>
            <TabsTrigger value="impostos" className="data-[state=active]:bg-green-600 data-[state=active]:text-white text-xs md:text-sm px-1 md:px-2 py-2 md:py-2.5 rounded-none">
              Impostos
            </TabsTrigger>
            <TabsTrigger value="resumo" className="data-[state=active]:bg-green-600 data-[state=active]:text-white text-xs md:text-sm px-1 md:px-2 py-2 md:py-2.5 rounded-none">
              Resumo
            </TabsTrigger>
            <TabsTrigger value="relatorio" className="data-[state=active]:bg-green-600 data-[state=active]:text-white text-xs md:text-sm px-1 md:px-2 py-2 md:py-2.5 rounded-none">
              Relatório
            </TabsTrigger>
          </TabsList>

        {/* Conteúdo das Abas - Scrollável */}
          <div className="h-[calc(100vh-280px)] md:h-[calc(95vh-280px)]">
            {/* Aba Cabeçalho */}
            <TabsContent value="cabecalho" className="space-y-3 p-4 mt-0 h-full overflow-y-auto">
        ...
        {/* Aba Catálogo de Produtos */}
          <TabsContent value="catalogo" className="space-y-3 p-4 mt-0 h-full overflow-y-auto">
          <CatalogoProdutosPedido
            ...
            onAdicionarItem={(produto, quantidade, desconto) => {
              ...
              const vlrUnitario = produto.VLRUNIT || produto.preco
              const vlrSubtotal = vlrUnitario * quantidade
              const vlrDesconto = desconto ? (vlrSubtotal * desconto) / 100 : 0
              const vlrTotal = vlrSubtotal - vlrDesconto
              ...
              // Chamar a função passada via props se ela existir
              if (onAdicionarItem) {
                onAdicionarItem(produto, quantidade, desconto);
              }
              
              // REMOVIDO:setShowProdutoModal(true) ou qualquer redirecionamento que abra modal indesejado
            }}
            tabelaPreco={pedido.CODTIPVENDA}
            itensCarrinho={itens || []}
        ...
        {/* Aba Mix de Produtos IA */}
          <TabsContent value="mixprodutos" className="space-y-3 p-4 mt-0 h-full overflow-y-auto">
            <MixProdutosIA
              codParc={pedido.CODPARC}
              nomeParceiro={pedido.RAZAO_SOCIAL}
              onAdicionarItem={(produto, quantidade, desconto) => {
