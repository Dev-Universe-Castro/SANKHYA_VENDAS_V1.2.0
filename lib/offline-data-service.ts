...
export class OfflineDataService {
      ...
      if (prefetchData.parceiros?.data) {
        promises.push(
          db.parceiros.clear().then(() => 
            db.parceiros.bulkAdd(prefetchData.parceiros.data)
          ).then(() => 
  ...
  static async getPrecos(codProd: number, nutab?: number) {
    try {
      console.log('🔍 getPrecos chamado - CODPROD:', codProd, '(tipo:', typeof codProd, ') NUTAB:', nutab, '(tipo:', typeof nutab, ')');

      // Verificar se a tabela de preços tem dados
      const totalPrecos = await db.precos.count();
      console.log('📊 Total de registros na tabela precos:', totalPrecos);

      if (totalPrecos === 0) {
        console.warn('⚠️ Tabela de preços vazia!');
        return [];
      }

      // Buscar amostras para debug
      const amostra = await db.precos.limit(3).toArray();
      console.log('📋 Amostra de 3 registros de preços:', amostra);

      if (nutab) {
        // Tentar busca com índice composto
        console.log('🔍 Tentando busca com índice composto [CODPROD+NUTAB]');
        let precos = await db.precos.where('[CODPROD+NUTAB]').equals([codProd, nutab]).toArray();
        console.log('📊 Preços encontrados com índice composto:', precos.length);

        // Se não encontrou, tentar busca alternativa
        if (precos.length === 0) {
          console.log('🔍 Tentando busca alternativa filtrando manualmente...');
          const todosPrecoProduto = await db.precos.where('CODPROD').equals(codProd).toArray();
          console.log('📊 Todos os preços do produto:', todosPrecoProduto.length);

          precos = todosPrecoProduto.filter(p => {
            const match = Number(p.NUTAB) === Number(nutab);
            if (match) {
              console.log('✅ Match encontrado:', p);
            }
            return match;
          });
          console.log('📊 Preços após filtro manual:', precos.length);
        }

        if (precos.length > 0) {
          console.log('💰 Preço(s) encontrado(s):', precos);
          console.log('💵 VLRVENDA:', precos[0].VLRVENDA, '(tipo:', typeof precos[0].VLRVENDA, ')');
        } else {
          console.warn('⚠️ Nenhum preço encontrado para CODPROD:', codProd, 'NUTAB:', nutab);
          // Mostrar quais NUTABs existem para este produto
          const precosDisponiveis = await db.precos.where('CODPROD').equals(codProd).toArray();
          if (precosDisponiveis.length > 0) {
            const nutabsDisponiveis = [...new Set(precosDisponiveis.map(p => p.NUTAB))];
            console.log('📋 NUTABs disponíveis para este produto:', nutabsDisponiveis);
          }
        }
      ...
      const precos = await db.precos.where('CODPROD').equals(codProd).toArray();
      console.log('📊 Todos os preços do produto:', precos.length);
      return precos;
    } catch (error) {
