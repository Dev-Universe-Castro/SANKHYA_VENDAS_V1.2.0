...
export async function criarPedidoVenda(pedido: PedidoVenda & { idEmpresa: number }): Promise<any> {
  try {
    ...
    // Obter credenciais para verificar o tipo de autenticação
    const credentials = await contratosService.getSankhyaCredentials(idEmpresa);

    // SEMPRE usar requestBody para o endpoint CRUDServiceProvider.saveRecord
    const corpoPedido = { requestBody: dadosPedido };
    ...
    console.log(`\n📦 CORPO JSON COMPLETO (com requestBody para CRUDServiceProvider):`);
    ...
    const endpoint = '/gateway/v1/mge/service.sbr?serviceName=CRUDServiceProvider.saveRecord&outputType=json';
    ...
    // Se retornou transactionId, buscar o NUNOTA usando loadRecords
    if (!nunota && resposta?.transactionId) {
      console.log(`\n🔍 ${credentials.authType} - Pedido criado, buscando NUNOTA...`);
      ...
      try {
        ...
        // Buscar usando loadRecords com MAX(NUNOTA) para obter o último registro do parceiro
        ...
        const searchPayload = {
          ...
          requestBody: {
            dataSet: {
              ...
              criteria: {
                expression: {
                  $: `this.NUNOTA = (SELECT MAX(NUNOTA) FROM TGFCAB WHERE CODPARC = ${pedido.CODPARC})`
                }
              }
            }
          }
